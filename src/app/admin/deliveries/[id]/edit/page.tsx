...
export default function EditDeliveryPage() {
    ...
    Promise.all([
      ...
      if (order) {
        setFormData({
          ...
          pickupLat: order.pickupLat ? Number(order.pickupLat) : undefined,
          pickupLng: order.pickupLng ? Number(order.pickupLng) : undefined,
          dropoffAddress: order.dropoffAddress || '',
          dropoffLat: order.dropoffLat ? Number(order.dropoffLat) : undefined,
          dropoffLng: order.dropoffLng ? Number(order.dropoffLng) : undefined,
  ...
  const handleCustomerSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (selected) {
      ...
      setFormData(prev => ({ ...prev, customerId: '', customerName: '', customerContact: '', dropoffAddress: prev.dropoffAddress }));
    }
  };
...
const inputStyle = { width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '14px', background: 'white', boxSizing: 'border-box' as const };
