              ...
              customers.map((c: any) => (
                <tr key={c.id}>
                  <td>
                    ...
                    <div style={{ fontSize: '12px', color: '#64748b' }}>{c.customerCode}</div>
                  </td>
                  <td>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                      <span style={{ fontSize: '13px', color: '#475569', display: 'flex', alignItems: 'center', gap: '4px' }}><Phone size={12}/> {c.mobileNumber}</span>
                      {c.email && <span style={{ fontSize: '13px', color: '#64748b', display: 'flex', alignItems: 'center', gap: '4px' }}><Mail size={12}/> {c.email}</span>}
                    </div>
                  </td>
                  <td>
                    <div style={{ fontSize: '13px', color: '#475569', display: 'flex', alignItems: 'flex-start', gap: '4px' }}>
                      ...
                      <span style={{ maxWidth: '200px' }}>{c.address}, {c.barangay}, {c.municipality}</span>
                    </div>
                  </td>
                  <td>
                    <span className={`${styles.badge} ${c.status === 'ACTIVE' ? styles.badgeActive : styles.badgeError}`}>
                      {c.status}
                    </span>
                  </td>
                  <td>
                    <button onClick={() => openEdit(c)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#3b82f6', padding: '4px 8px' }}><Edit2 size={16} /></button>
                    <button onClick={() => handleDelete(c.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444', padding: '4px 8px' }}><Trash2 size={16} /></button>
                  </td>
                </tr>
              ))
            )}
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '32px', width: '100%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                ...
                <textarea className={styles.inputField} style={{ minHeight: '80px' }} value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} />
              </div>
