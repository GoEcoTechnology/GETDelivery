...
export default function PartnerOrderActions({ orderId, status }: { orderId: number; status: string }) {
  ...
  const isAssigned = status === 'ACCEPTED' || status === 'ASSIGNED' || status === 'TEMPORARY_WINNER' || status === 'IN_TRANSIT' || status === 'DELIVERED';
    ...
    return (
      <div style={{ display: 'grid', gap: '16px' }}>
        <div style={{ padding: '16px 20px', borderRadius: '18px', border: '1px solid #e2e8f0', background: '#fff', display: 'flex', justifyContent: 'center' }}>
          {status === 'DELIVERED' && <StateMessage icon={<CheckCircle2 size={18} />} color="#16a34a" text="You completed this delivery" />}
          {['ACCEPTED', 'ASSIGNED', 'TEMPORARY_WINNER'].includes(status) && <StateMessage icon={<CheckCircle2 size={18} />} color="#16a34a" text="You have been assigned!" />}
          {status === 'IN_TRANSIT' && <StateMessage icon={<CheckCircle2 size={18} />} color="#4f46e5" text="You are delivering this order" />}
