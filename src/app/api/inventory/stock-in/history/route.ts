...
import { db } from '@/db';
import { stockIns, products, users } from '@/db/schema';
...
export async function GET(request: Request) {
  return withAuth(request, { requiredPermissions: ['inventory.view'] }, async (tx, claims) => {
    ...
    const conditions = [eq(stockIns.tenantId, tenantIdToUse)];

    if (search) {
      conditions.push(
        or(
          ilike(stockIns.referenceNumber, `%${search}%`),
          ilike(stockIns.supplier, `%${search}%`)
        )!
      );
    }

    if (startDate) {
      conditions.push(gte(stockIns.createdAt, new Date(startDate)));
    }
    
    if (endDate) {
      const end = new Date(endDate);
      end.setHours(23, 59, 59, 999);
      conditions.push(lte(stockIns.createdAt, end));
    }

    // Get unique reference numbers (pagination based on distinct transactions)
    const distinctTx = await tx
      .select({
        referenceNumber: stockIns.referenceNumber,
      })
      .from(stockIns)
      .where(and(...conditions))
      .groupBy(stockIns.referenceNumber, stockIns.createdAt)
      .orderBy(desc(stockIns.createdAt))
      .limit(limit)
      .offset(offset);

    const countResult = await tx
      .select({ count: sql`count(distinct ${stockIns.referenceNumber})`.mapWith(Number) })
      .from(stockIns)
      .where(and(...conditions));
    ...
    const rawData = await tx
      .select({
        ...
        unitCost: stockIns.unitCost,
        supplier: stockIns.supplier,
        notes: stockIns.notes,
        employeeName: users.name,
        createdAt: stockIns.createdAt,
        status: stockIns.status
      })
      .from(stockIns)
      .leftJoin(products, eq(stockIns.productId, products.id))
      .leftJoin(users, eq(stockIns.performedBy, users.id))
      .where(sql`${stockIns.referenceNumber} IN (${sql.join(references, sql`, `)})`)
      .orderBy(desc(stockIns.createdAt));
